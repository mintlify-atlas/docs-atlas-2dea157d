...
description: Guidelines for contributing code to PocketMine-MP
---
...
<Tip>
Want to contribute (non-English) translations? Visit the [Crowdin Translation Project](https://crowdin.com/project/pocketmine) instead.
...
| Removal of API | ❌ | ❌ | ✔️ |
| Backwards-incompatible API change | ❌ | ❌ | ✔️ |
| Backwards-incompatible internals change | ❌ | ✔️ | ✔️ |
...
- **API** includes all public and protected classes, functions and constants (unless marked as `@internal`).
  ...
  - The `pocketmine\network\mcpe` package is considered implicitly `@internal` in its entirety.
...
- BC-breaking changes to the internal network API are allowed, but only in new minor versions.

## Making a Pull Request

<Steps>
  <Step title="Fork the repository">
    [Fork the repository on GitHub](https://github.com/pmmp/PocketMine-MP/fork). This gives you your own copy of the repository to make changes to.
  </Step>
  ...
  <Step title="Submit the pull request">
    You can then make a [pull request](https://github.com/pmmp/PocketMine-MP/pull/new) to the project.
  </Step>
</Steps>
...
- **All code must be licensed under the [LGPLv3 license](https://github.com/pmmp/PocketMine-MP/blob/stable/LICENSE) or a compatible license.**
...
- **Use `final`, `private` and `readonly` wherever possible**. This allows us to change more things later on if needed without breaking plugins.
- **Do not add unnecessary setters or public writable properties to classes.** Immutable things are preferred because their behaviour is more predictable.
...
<Tip>
Don't worry about getting a PR perfect on the first try. In fact, it's quite unusual for a PR to be perfect when it's first submitted, and most PRs will get changes requested by reviewers, even when the PR is made by one of our team members.

Mistakes are normal, and PMMP team members will review your code and suggest changes to your code as needed. Just make sure to stick with it so you can communicate with reviewers and/or make changes.
</Tip>
...
- **Run quality tools like PHPStan locally before you commit.** See the [Testing and Quality Tools](/contributing/testing) page.
...
RFCs should be submitted using Issues or Discussions. RFCs _can_ be submitted as pull requests if you've already written the code, but this is not recommended, since it's not guaranteed that an RFC will pass, in which case your effort would be wasted.
...
Community members can vote on RFCs. This gives maintainers an idea of how popular the idea is. Votes can be cast using :+1: and :-1: reactions.
...
## PocketMine Namespace Dependencies

PocketMine-MP has several dependencies which are independent from the main server code. Most of them use the `pocketmine` namespace. Some of these add extra classes to packages which already exist in PocketMine-MP.
